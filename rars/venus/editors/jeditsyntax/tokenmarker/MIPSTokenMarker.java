...
import rars.venus.editors.jeditsyntax.*;
...
public class MIPSTokenMarker extends TokenMarker {
    ...
    public static KeywordMap getKeywords() {
        if (cKeywords == null) {
            ...
            java.util.ArrayList instructionSet = rars.Globals.instructionSet.getInstructionList();
            for (int i = 0; i < instructionSet.size(); i++) {
                cKeywords.add(((rars.mips.instructions.Instruction) instructionSet.get(i)).getName(), Token.KEYWORD1);
            }
            // add assembler directives
            java.util.ArrayList directiveSet = rars.assembler.Directives.getDirectiveList();
            for (int i = 0; i < directiveSet.size(); i++) {
                cKeywords.add(((rars.assembler.Directives) directiveSet.get(i)).getName(), Token.KEYWORD2);
            }
            // add integer register file
            rars.mips.hardware.Register[] registerFile = rars.mips.hardware.RegisterFile.getRegisters();
            for (int i = 0; i < registerFile.length; i++) {
                cKeywords.add(registerFile[i].getName(), Token.KEYWORD3);
                cKeywords.add("x" + i, Token.KEYWORD3);  // also recognize $0, $1, $2, etc
            }
            // add Coprocessor 1 (floating point) register file
            rars.mips.hardware.Register[] coprocessor1RegisterFile = rars.mips.hardware.Coprocessor1.getRegisters();
    ...
    public ArrayList getTokenExactMatchHelp(Token token, String tokenText) {
        ...
        if (token != null && token.id == Token.KEYWORD1) {
            ArrayList instrMatches = rars.Globals.instructionSet.matchOperator(tokenText);
            if (instrMatches.size() > 0) {
                int realMatches = 0;
                matches = new ArrayList();
                for (int i = 0; i < instrMatches.size(); i++) {
                    Instruction inst = (Instruction) instrMatches.get(i);
                    if (rars.Globals.getSettings().getExtendedAssemblerEnabled() || inst instanceof BasicInstruction) {
    ...
    public ArrayList getTokenPrefixMatchHelp(String line, Token tokenList, Token token, String tokenText) {
        ...
        if (token != null && token.id == Token.NULL) {
            ...
            if (keywordTokenText == null && trimmedTokenText.length() > 0) {
                ...
                } else if (trimmedTokenText.length() >= rars.Globals.getSettings().getEditorPopupPrefixLength()) {
    ...
    private ArrayList getTextFromInstructionMatch(String tokenText, boolean exact) {
        ...
        if (exact) {
            matches = rars.Globals.instructionSet.matchOperator(tokenText);
        } else {
            matches = rars.Globals.instructionSet.prefixMatchOperator(tokenText);
        }
        if (matches == null) {
            return null;
        }
